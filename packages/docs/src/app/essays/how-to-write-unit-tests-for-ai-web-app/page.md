...
In order to test retrieval in the prior section, we had to review each record in the DB and mark it as relevant or not to the user query. This is not feasible in a real-life scenario, in which a knowledge database could have thousands or millions or records.
...
Poyro provides a function [outputFulfillsCriterion](/sdk-reference/output-fulfills-criterion) that we can use to calculate both contexual precision and contexual recall:
...
describe("meets precision and recall targets", async () => {
  // Note that all statements in the context are present in the output.
  // Thus this variable will be equal to 3 after the loop ends.
  var numStatementsAttributableToContext = 0;

  for (idealOutputStatement of idealOutputStatements) {
    for (actualContext of actualContexts) {
      const isStatementAttributableToContext = await outputFulfillsCriterion(
        idealOutputStatement,
  ...
  it("meets precision target", () => {
    // (3 statements in ideal output attributable to context) / (3 statements in context) = 100%
  ...
  it("meets recall target", () => {
    // (3 statements in ideal output attributable to context) / (4 statements in ideal output) = 75%
...
We can again use the function [outputFulfillsCriterion](/sdk-reference/output-fulfills-criterion) to calculate these metrics:
...
describe("meets relevancy and faithfulness targets", async () => {
  // 3 statements in the output reference Einstein, so relevant to query
  var numStatementsRelevantToQuery = 0;
  // 1 statement in the output matches any in the context, others hallucianted
  var numStatementsAttributableToContext = 0;

  for (actualOutputStatement of actualOutputStatements) {
    // Determine whether this statement in the ouput is relevant to the user query
    const isStatementRelevantToQuery = await outputFulfillsCriterion(
        actualOutputStatement,
        "output information can be attributed is relevant to user query passed in additional context",
        query,
    );

    if (isStatementRelevantToQuery) {
      numStatementsRelevantToQuery += 1;
    }

    // Check if output statement attributable to any fact in context
    for (actualContext of actualContexts) {
      isStatementAttributableToContext = await outputFulfillsCriterion(
        idealOutputStatement,
        "output information can be attributed to information in additional context",
        actualContext,
      );
  ...
  it("meets relevancy target", () => {
    // 3 statement relevant to user input) / (4 statements in output) = 75%
...
To increase relevance we could emphasize in the prompt that the LLM response should only contain facts about the historical figure in the user query. To increase the faithfulness we may want to use a better model since the current model is doing pretty bad here.
...
In cases where we have a single binary condition, our best option is Poyro's [toFulfillCriterion](/sdk-reference/to-fulfill-criterion) matcher. Let's say we have a legal assistance app, that explains legal terminology to young adults getting their first job:

```js
describe("AI legal assistance is understandable to young layperson", async () => {
...
```js
const aiLegalAdvise = aiLegalAdvisor("Tell me a best practice regarding treatment of information in an NDA");
// Returns: `It is recommended that both parties clearly define what information should be labeled "Confidential".`
```

is attributable to one of these best practices in the file:

```js
describe("can attribute legal recommendation to some best practice in file", () => {
  var isAttributable = false;
  for(practice of bestPractices) {
    isAttributable = await outputFulfillsCriterion(
      aiLegalAdvise,
...
This is similar to the faithfulness metric we discussed in the prior section, except that in this case we just care to attribute the output to at least 1 data in our database.
