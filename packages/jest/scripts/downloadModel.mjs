...
const main = async () => {
  ...
  if (!res.ok) {
    throw new Error(`@poyro/jest: Failed to fetch ${url}: ${res.statusText}`);
  }
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (fs.statSync(filePath).size === totalBytes) {
      console.log("@poyro/jest: Model already exists, skipping download.");
      return filename;
    }
  ...
  console.log(`@poyro/jest: Downloading '${filename}'...`);
...
main()
  .then((filename) => {
    console.log(`@poyro/jest: Model download complete: '${filename}"`);
  })
  .catch((err) => {
    console.error("@poyro/jest: Error downloading file:", err);
    process.exit(1);
  });
